...
`composition-policy-v2`는 WealthCopy가 설계한 L1–L15별 내부 참고범위입니다. 여섯 개 앵커 사이를 서버에서 보간해 열다섯 단계가 서로 다른 구성 범위를 갖습니다. 외부 가구 데이터의 공개 백분위 좌표는 분포 간격을 보정하는 백데이터로만 사용하며, 달러 금액을 원화로 바꾸거나 한국 자산순위를 추정하지 않습니다. 데이터셋 이름·원시 금액·백분위 좌표는 앱, 공개 API, 모델 입력에 노출하지 않습니다.
...
`level-route-policy-v1`은 L1의 순자산 회복부터 L15의 장기 운영·영속성까지 각 레벨에 서로 다른 경로 목적과 세 구간을 제공합니다. L15는 다음 레벨을 만들지 않으며, 모든 경로는 도달 시점이나 수익을 약속하지 않습니다.
