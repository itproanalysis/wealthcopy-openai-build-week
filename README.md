...
[Live demo](https://wealth-copy-470320899177.asia-northeast3.run.app/) · [Submission package](submission/README.md) · [Deployment evidence](docs/GCP_DEPLOYMENT.md)
...
No login or account connection is required. The Korean labels and their English meanings used in the demo are documented in [testing instructions](submission/03_TESTING_INSTRUCTIONS_EN.md).
...
Every model choice must come from the context-specific allowlist. The server rejects the entire plan if an ID is missing, extra, or invalid, then returns the same strict report shape through a deterministic fallback. OpenAI Responses use Structured Outputs, `store: false`, low reasoning effort, a short timeout, no SDK retries, and a hashed safety identifier.

This design gives each household a more relevant reading order without asking a generative model to calculate or recommend financial outcomes.
...
The current local candidate passes lint, type checking, 54 tests, and a production build. Test coverage includes all fifteen bands, all eight asset groups, threshold edges, safety stops, strict request rejection, minimized model input, fallback parity, no-store headers, and removal of the legacy v2 endpoint.
